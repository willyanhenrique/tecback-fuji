...
public class AtorController {
    ...
    private final AtorService atorService; // Serviço para Ator
    ...
    public ResponseEntity<AtorDTO> criarAtor(@RequestBody AtorDTO atorDTO) {
        ...
        return ResponseEntity.status(201).body(atorCriado); // 201 Created
    }
    ...
    public ResponseEntity<Void> deletarAtor(@PathVariable Long id) {
        ...
        if (deletado) {
            log.info("Ator com id: {} deletado com sucesso", id);
            return ResponseEntity.noContent().build();
        } else {
            log.warn("Tentativa de deletar ator com id: {} não encontrado", id);
